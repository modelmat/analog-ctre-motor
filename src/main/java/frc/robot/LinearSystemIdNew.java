...
public final class LinearSystemIdNew {
  private LinearSystemIdNew() {
    // Utility class
  }
  ...
  /**
   * Create a state-space model of a spinny motor system. The states of the system are [angular
   * position, angular velocity], inputs are [voltage], and outputs are [angular position,
   * angular velocity].
   *
   * @param motor The motor (or gearbox) attached to the spinny motor.
   * @param jKgMetersSquared The moment of inertia J of the spinny motor.
   * @param G The reduction between motor and drum, as a ratio of output to input.
   * @return A LinearSystem representing the given characterized constants.
   * @throws IllegalArgumentException if jKgMetersSquared &lt;= 0 or G &lt;= 0.
   */
  @SuppressWarnings("ParameterName")
  public static LinearSystem<N2, N1, N2> createSpinnyMotorSystem(
      DCMotor motor, double jKgMetersSquared, double G) {
    if (jKgMetersSquared <= 0.0) {
      throw new IllegalArgumentException("J must be greater than zero.");
    }
